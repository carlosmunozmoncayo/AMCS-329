...
def integrate_f_phi(shape_function,affine_mapping,f,degree):
    def compose_f_phi_affine(x_ref,y_ref):
        x = x_ref*affine_mapping.A[:,0] + y_ref*affine_mapping.A[:,1] + affine_mapping.b
        x,y = x
        return f(x,y)*shape_function.phi((x,y))
...
def integrate_gradphiI_dot_gradphiJ(shapef_i,shapef_j,affine_mapping,degree):
    def compose_grad_phi_affine(x_ref,y_ref):
        x = x_ref*affine_mapping.A[:,0] + y_ref*affine_mapping.A[:,1] + affine_mapping.b
        x,y = x
        return np.dot(shapef_i.grad_phi((x,y)),shapef_j.grad_phi((x,y)))
